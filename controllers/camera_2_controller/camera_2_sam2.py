...
box1 = np.array([1760, 855, 1936, 992], dtype=np.float32)
box2 = np.array([1694, 1102, 1980, 1184], dtype=np.float32)
box3 = np.array([1764, 686, 1926, 822], dtype=np.float32)
box4 = np.array([2006, 767, 1931, 2151], dtype=np.float32)
box5 = np.array([1598, 766, 1698, 933], dtype=np.float32)
...
labels[num_points_box1 + num_points_box2 + num_points_box3:] = 0
...
def show_masks(image, masks, scores,name, point_coords=None, box_coords=None, input_labels=None, borders=True):
    for i, (mask, score) in enumerate(zip(masks, scores)):
        ...
        plt.axis('off')
        plt.show()
        # plt.savefig(f'mask_frame_{name:04d}.jpg')
...
# 获取摄像头设备
camera = robot.getDevice('camera_2')
